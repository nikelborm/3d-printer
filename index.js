...
const printerPasswordHash =
  process.env.PRINTER_PASSWORD_HASH ||
  '2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824';
...
const sendToAuthorized = body =>
  broadcast(body, client => client.isUser || client.isPrinter);
  ...
  switch (data.event) {
    ...
    case 'sendGCodeFile': {
      ...
      filePrintingInfo = {
        ...
        startTime: Date.now(),
        finishTime:
          Date.now() + (data.secondsCost + secondsForHeatingExtruder) * 1000,
      };
      sendToUsers({
...
webSocketServer.on('connection', (connection, request) => {
  ...
  connection.addListener('message', async function (input) {
    ...
    allMessagesHandler(JSON.parse(input.toString()), connection);
